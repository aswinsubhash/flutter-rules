...
function pluginExists(runner, command, scope) {
  return hasPluginEntry(pluginList(runner, command), scope);
}
...
function marketplaceSourceOverride(env) {
  return env.FLUTTER_RULES_MARKETPLACE_SOURCE
    || env.FLUTTER_RULES_CODEX_MARKETPLACE_SOURCE
    || null;
}

function marketplaceSource(env) {
  return marketplaceSourceOverride(env) || CODEX_SOURCE;
}

function installCodex({ runner, env, mode }) {
  requireCommand(runner, 'codex', 'Install Codex, then rerun this command.');
  const source = marketplaceSource(env);
  const override = marketplaceSourceOverride(env);
  const configured = marketplaceExists(runner, 'codex');
  if (configured) {
    if (override) {
      throw new Error('Codex marketplace is already configured; refusing to replace its source automatically. Reconfigure it explicitly, then rerun this command.');
    }
    runRequired(runner, 'codex', ['plugin', 'marketplace', 'upgrade', CODEX_MARKETPLACE]);
  } else {
    const args = ['plugin', 'marketplace', 'add', source];
    if (!override && !localSource(source)) args.push('--ref', 'main');
...
function installClaude({ runner, env, mode }) {
  requireCommand(runner, 'claude', 'Install Claude Code, then rerun this command.');
  const source = marketplaceSource(env);
  const override = marketplaceSourceOverride(env);
  const configured = marketplaceExists(runner, 'claude');
  if (configured) {
    if (override) {
      throw new Error('Claude marketplace is already configured; refusing to replace its source automatically. Reconfigure it explicitly, then rerun this command.');
    }
    runRequired(runner, 'claude', ['plugin', 'marketplace', 'update', CODEX_MARKETPLACE]);
  } else {
    runRequired(runner, 'claude', ['plugin', 'marketplace', 'add', source]);
  }
  const action = pluginExists(runner, 'claude', 'user') ? 'update' : 'install';
  runRequired(runner, 'claude', ['plugin', action, CODEX_PLUGIN, '--scope', 'user']);
...
function uninstallClaude({ runner }) {
  ...
  const hasOtherScope = matchingPluginEntries(plugins).some((entry) =>
...
function doctorClaude({ runner }) {
  requireCommand(runner, 'claude', 'Install Claude Code, then rerun this command.');
  return pluginExists(runner, 'claude')
...
function installFilesystemSkill({ home, packageRoot, dryRun, mode, log, platform }) {
  const source = join(packageRoot, 'src', 'flutter-rules');
  const destination = platform === 'cursor' ? cursorDestination(home) : devinDestination(home);
  ...
  const label = platform === 'cursor' ? 'Cursor' : 'Devin Local';
  return `${action} ${label} skill at ${destination}.`;
}

function installCursor(context) {
  return installFilesystemSkill({ ...context, platform: 'cursor' });
}
...
function installDevin(context) {
  return installFilesystemSkill({ ...context, platform: 'devin' });
}
...
function hasExplicitInvocationMetadata(content) {
  try {
    return frontmatterField(content, 'disable-model-invocation') === 'true'
      && frontmatterField(content, 'triggers') === '["user"]';
  } catch {
    return false;
  }
}
...
function doctorFilesystemSkill({ home, target }) {
  ...
  const ownVersion = skillVersion(ownContent);
  if (!ownVersion) return `${label} skill version metadata is missing. Run \`flutter-rules update all\`.`;
  ...
  const otherVersion = skillVersion(otherContent);
  if (!otherVersion) {
    return `Cursor and Devin skill version metadata is missing. Run \`flutter-rules update all\`.`;
  }
  if (ownVersion !== otherVersion) {
    return `Cursor and Devin skill versions differ (${ownVersion} vs ${otherVersion}). Run \`flutter-rules update all\`.`;
  }
