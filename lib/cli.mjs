...
import { commandExists, createCommandRunner } from './command-runner.mjs';
...
function skipAllTarget({ command, context, target }) {
  return target === 'claude'
    && ['install', 'update'].includes(command)
    && !context.claudeAvailable
    && !existsSync(join(claudeDestination(context.home), 'SKILL.md'));
}

async function runAll({ command, context, log, error }) {
  const results = [];
  for (const target of ['agents', 'claude']) {
    if (skipAllTarget({ command, context, target })) {
      const message = 'Skipped because the Claude Code CLI was not detected.';
...
export async function runCli(argv, {
  ...
  const claudeAvailable = runner ? runner.has('claude') : commandExists('claude', env);
