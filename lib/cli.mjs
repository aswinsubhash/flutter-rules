...
function preflightCanonical({ home, manager }) {
  ...
  if (entry && !status.valid && (entry.source || entry.sourceUrl)) {
...
function doctorState({ env, home, manager, runner, version }) {
  ...
  const sourceValid = status.valid;
  const claudePath = claudeDestination(home, env);
...
function doctor(context, json) {
  ...
  if (!state.canonical.sourceValid) context.error('Issue: Skills CLI source is missing or unexpected.');
