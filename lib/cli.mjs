...
const DEVIN_SETUP = `Devin Local user-level installation:

  flutter-rules install devin

Devin cloud sessions use organization-level repository discovery:
...
function doctorDevin({ home }) {
  const skillPath = join(devinDestination(home), 'SKILL.md');
  return existsSync(skillPath) ? 'Devin Local skill is installed.' : 'Devin Local skill is not installed.';
}
