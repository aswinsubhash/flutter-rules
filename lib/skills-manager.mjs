...
export function sourceStatus(entry, expectedSource) {
  if (!entry) return { valid: false, source: null, ref: null, local: false };
  if (existsSync(expectedSource)) {
    return { valid: true, source: expectedSource, ref: null, local: true };
  }
  ...
  return {
    valid: (recordedRepository === normalizedRepository || sourceUrlRepository === normalizedRepository)
      && (!ref || entry.ref === ref),
    source: repository,
    ref: entry.ref ?? null,
    local: false,
